(() => {
    ...
    (() => {
            ...
            optionAction(selectItem, originalSelect, optionItem) {
                ...
                if (!selectOptions.classList.contains("_slide")) {
                    if (originalSelect.multiple) {
                        ...
                        if (originalSelect.hasAttribute("data-checkbox")) this.setSelectTitleValue(selectItem, originalSelect, selectSelectedItems.length); else this.setSelectTitleValue(selectItem, originalSelect);
                    } else {
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) new swiper_core_Swiper(".popup-campaign__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                spaceBetween: 12,
        ...
        function cardTags() {
            document.addEventListener("DOMContentLoaded", (() => {
                ...
                cardTagsList.forEach((cardTags => {
                    const cardTagSpans = cardTags.querySelectorAll(".card-tag");
            ...
            document.addEventListener("DOMContentLoaded", (() => {
                ...
                filters.forEach((filter => {
                    ...
                    if (buttonFilter && filterMore) {
                        const isFilterMoreVisible = localStorage.getItem("filterMoreVisible");
                        if (isFilterMoreVisible === "true") {
                            filterMore.removeAttribute("hidden");
                            buttonFilter.classList.add("active");
                        } else {
                            filterMore.setAttribute("hidden", "true");
                            buttonFilter.classList.remove("active");
                        }
                        buttonFilter.addEventListener("click", (() => {
                            _slideToggle(filterMore);
                            const isVisible = filterMore.style.display !== "none" ? "true" : "false";
                            localStorage.setItem("filterMoreVisible", isVisible);
        ...
        window.addEventListener("load", updateProgressWidth);
        function updateProgressWidth() {
            var progress = document.getElementById("progress");
            var activeStep = document.querySelector(".progress-step.active");
            var levelBar = document.querySelector(".level-bar");
            if (progress && activeStep && levelBar) {
                var levelBarOffsetLeft = levelBar.getBoundingClientRect().left;
                var activeStepOffsetLeft = activeStep.getBoundingClientRect().left;
                var progressWidthPercent = (activeStepOffsetLeft - levelBarOffsetLeft) / levelBar.offsetWidth * 100;
                progressWidthPercent = Math.min(progressWidthPercent, 100);
