(() => {
    ...
    (() => {
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            modules_flsModules.tippy = tippy_esm(".progress-step", {
                content(reference) {
                    const id = reference.getAttribute("data-template");
                    const template = document.getElementById(id);
                    if (template) return template.innerHTML; else return "Template not found";
                },
                allowHTML: true,
                onShow(instance) {
                    const isActive = instance.reference.classList.contains("actived");
                    instance.popper.classList.add("levelbar-tooltip");
                    if (isActive) instance.popper.classList.add("type-done");
                }
            });
        }));
